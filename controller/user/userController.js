...
exports.userLogin = asyncErrorHandler(async (req, res, next) => {
  ...
  const user = await User.findOne({ email }).populate({
    ...
    FetchedUser.role = user.role_id.name
...
exports.userCreate = asyncErrorHandler(async (req, res, next) => {
  const { name, email, in_active, company, role } = req.body;
  const user_photo = req.imagePath || null;

    // Validation: Check if required fields are provided
    if (!name || !email || !in_active || !company === undefined) {
...
exports.getPermissionsOfUser = asyncErrorHandler(async (req, res, next) => {
  const pool = req.pool;
  const { id } = req.query
